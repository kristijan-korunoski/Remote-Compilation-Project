...
char *convert_to_full_line(char **line_array, int line_len, int nwords)
{
  char *line;
  ...
  strcpy(line, "[");
  strcat(line, "'");
  memmove(line_array[0], line_array[0] + 8, strlen(line_array[0]));
  if (line_len == 1)
  {
    strcat(line, line_array[0]);
...
void send_to_server(int server, char *send_data)
{
  ...
  char *ack;
  char *msg;
  send(server, send_data, strlen(send_data), 0);
  ...
  if (starts_with(msg, "\\FileTransfer")) // recieve file time
  {
    char *filename;
    ...
    printf("~~ client received last ack - %s ~~\n", server_message);
  }
...
int main(int argc, char *argv[])
{
  ...
  while (n < numOfLines)
  {
    ...
    else if (starts_with(line[0], "\tremote-"))
    {
      ...
      printf("n: %i num: %i\n", n, num);
      printf("array host: %d\n", arrayOfHosts[0]);
      // char *pass_line = convert_to_full_line(line, line_lengths[n], line_lengths[num] - 1);

      char pass_line[250];

      memmove(pass_line, pass_line + 8, strlen(pass_line));
      strcpy(pass_line, "[");
      strcat(pass_line, "'");
      memmove(line[0], line[0] + 8, strlen(line[0]));

      if ((line_lengths[num]) == 1)
      {
        printf("YES\n");
        strcat(pass_line, line[0]);
      ...
      else
      {
        ...
        strcat(pass_line, line[0]);
        strcat(pass_line, "'");
        while (i != (line_lengths[num] - 1))
        {
          strcat(pass_line, ",");
          strcat(pass_line, "'");
          strcat(pass_line, line[i]);
          strcat(pass_line, "'");
          i++;
        }
        strcat(pass_line, ",");
      ...
      printf("~~~ next: %s ~~~~\n", all_lines[num][0]);
      printf("\n");
      char line_string[300] = "";
      strcat(line_string, pass_line);

      if (num < numOfLines) // check that its not the end of the rakefile
      {
        if (starts_with(all_lines[num][0], "\t\trequires"))
        {
          printf("YES\n");
          printf("array host: %d\n", arrayOfHosts[0]);
          ...
          ack = server_message;
          int numoffiles = line_lengths[num] - 1;
          ...
          {
            char *filename = all_lines[num][filenumber];
      ...
      send_to_server(arrayOfHosts[0], line_string);
    }
    else
    {
      // subproccess
